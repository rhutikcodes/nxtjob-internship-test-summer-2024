...
export default function Comment({ roomId }: { roomId: string }) {
  ...
  async function sendComment(roomId: string, comment: string) {
    ...
    if (comment.length != 0) {
      console.log(roomId);
      const res = await axios.post(`${API_BASE_URL}/api/sendComment`, {
        roomId: roomId,
        comment: comment,
      });
      ...
      if (res?.data?.mssg) {
        try {
          ...
          await axios.put(
            " https://coding-task.shishirkj08.workers.dev/api/v1/addComment",
            { index: roomId, comment: comment }
          );
        } catch (error: unknown) {
          console.log(error);
        } finally {
    ...
    async function getComments() {
      try {
        const res = await axios.get(
          `https://coding-task.shishirkj08.workers.dev/api/v1/getComment/${roomId}`
        );
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            ...
            />
            <button
              onClick={() => sendComment(roomId, comment)}
              className="bg-pink-500 text-white font-medium py-2 px-4 rounded-r-lg hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500"
            >
              Comment
            </button>
          </div>
          <button className="text-gray-500 hover:text-gray-700 focus:outline-none">
            <XIcon
              onClick={() => dispatch(CommentComponent(roomId))}
              className="h-6 w-6"
            />
