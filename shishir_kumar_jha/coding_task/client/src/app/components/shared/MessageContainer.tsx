...
export default function MessageContainer() {
  ...
  let postContent = useSelector((state:RootState)=>state.postInfo.postContent)
  let showcreateCommentComponent = useSelector((state:RootState)=>state.userInfo.showcreateCommentComponent)
  let commentLength = useSelector((state:RootState)=>state.postInfo.comment.commentLength)
  let roomIdForComments = useSelector((state:RootState)=>state.postInfo.comment.roomId)
  ...
  useEffect(() => {
    if (postContent) {
      ...
      setPostArray(prevArray => [
        ...prevArray,
        { content: postContent, likes:0, parsedContentIdArray: [],userId:userId,createdAt:new Date().toISOString(),postId:new Date().toISOString()} 
      ]);
    }
...
async function fetchContent(){ 
  const res =  await axios.get("https://coding-task.shishirkj08.workers.dev/api/v1/getAllPosts")
...
const handleNewMessage = (message:{count:number,id:string}) => {
  const postIndex = postArray.findIndex(post => post.postId === message.id);
  if (postIndex !== -1) {
...
<div className=' flex items-center justify-between w-[182[px] h-[24px] gap-4'>
  <p className='w-[45px] h-[24px] text-base font-medium size-[16px] text-[#7047eb] '>{(content?.userId)?(content?.userId):"Anonymous"}</p>
  <p className='text-xs font-light size-[12px] w-[155px] h-[16px] text-[#8a8aa3]'>{content.createdAt?(content.createdAt):"11:15"}</p>
</div>
{/* mssg writeen long */}
<div className='w-[740px] h-[54px] gap-[10px]'>
  <p className='h-[54px] w-[740px] mt-1  font-inter font-normal text-sm leading-5 size-3.5 text-[#3f3f50]'>{(content.content)}</p>
</div>
{/* likes save  */}
...
<div className='w-[54px] h-[24px] gap-1 mt-4'>
  ...
  <p className='w-[26px] ml-2 h-[24px] text-sm leading-6 size-3.5 text-[#8a8aa3]'>{content.likes}</p>

 

<Image  onClick={()=>{toggleCommentComponent(parseInt(content.postId))}}  src={"chat.svg"} alt='chat' height={13} width={14.5} className='ml-5 cursor-pointer' />
...
<p className='w-[26px] ml-2 h-[24px] text-sm leading-6 size-3.5 text-[#8a8aa3]'>{content?.parsedContentIdArray?.length}</p>
  </div>
  </div>
</div>
...
<Loading/>
  ...
  {typeof showcreateCommentComponent==='string'? <Comment roomId={showcreateCommentComponent}/>:""}
