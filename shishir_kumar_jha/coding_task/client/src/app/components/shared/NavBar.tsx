...
export default function NavBar() {
  ...
  const [showNotificationInput, setshowNotificationInput] =
    useState<boolean>(false);
    ...
    async function getAllNotification() {
      try {
        const res = await axios.get(
          "https://coding-task.shishirkj08.workers.dev/api/v1/getAllNotification"
        );
        if (res.data.mssg) {
  ...
  async function handleNotification() {
    try {
      ...
      if (content.length != 0) {
        const res = await axios.post(`${API_BASE_URL}/api/sendNotification`, {
          content: content,
        });
        ...
        if (res?.data.mssg) {
          ...
          await axios.post(
            "https://coding-task.shishirkj08.workers.dev/api/v1/createNotification",
      ...
      <nav className="bg-[#FFFFFF] w-[1195px] h-[64px]  border-b-[1px] border-gray-200 flex justify-between left-[280.01px] p-[12px]">
        ...
        {!showNotificationInput && (
          <div
            ...
            className="cursor-pointer text-[#56c24d] "
          >
        ...
        {showNotificationInput && (
          <div className="flex items-center">
            <input
              ...
              value={content}
              className="bg-gray-200 text-black rounded-md h-7"
            />
            <Image
              onClick={() => setshowNotificationInput(!showNotificationInput)}
              className="absolute left-[48rem] cursor-pointer "
              src={"/cross.svg"}
              alt="x"
              width={12}
              height={12}
            ...
            <Button
              ...
              className=" bg-[#ff69b4] h-7 rounded-xl  leading-5 pb-[28px] ml-3 text-white "
            >
              send Notificaiton
            </Button>
          </div>
        ...
        <div className="w-[360px] h-[32px] gap-4">
          ...
          <div className="flex ">
            <div className="w-[240px] h-[32px] rounded-[0.25rem] pt-1 pr-2 pb-1 pl-2 bg-[#F7F7F8] flex justify-between">
              <p className="w-[40px] h-[24px] text-sm leading-6 size-[12px] text-[#8A8AA3] font-medium ">
                Search
              </p>
              <div className="w-[16px] h-[16px]">
                <Image
                  ...
                  width={13.14}
                  height={13.14}
                  className="top-[1.33px] mt-1.5 left-[1.33px]"
                />
              </div>
            </div>
            {/* Bookmark */}
            <div className=" flex w-[24px] h-[24px]">
              <Image
                ...
                height={13.5}
                width={12}
                className="top-[1.33px]  ml-5 mt-2 left-[1.33px]"
              />
              ...
              {showNotificationContainer && (
                <ul className="absolute right-[5px] top-[38px] z-10 min-w-[180px] overflow-auto rounded-md border border-blue-gray-50 bg-white p-3 font-sans text-sm font-normal text-blue-gray-500 shadow-lg shadow-blue-gray-500/10 focus:outline-none">
                  {notificationArray.length != 0 &&
                    notificationArray.map((element) => (
                      <div key={element.id}>
                  ...
                  <Button
                    ...
                    className="w-full bg-[#ff69b4] rounded-lg border-[1px] gap-[8px] leading-5"
                  >
                    Close
                  </Button>
                </ul>
              ...
              <Image
                ...
                height={20}
                width={20}
                className="top-[1.33px]  ml-10 mt-2 left-[1.33px]"
              />
            </div>
          </div>
        </div>
